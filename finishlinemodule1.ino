...
const float TARGET_FREQ = 150000.0;
const float FREQ_TOLERANCE = 50000.0;
...
// At 150kHz, period is ~6.67 microseconds, so 3 microsecond debounce
if (currentTime - lastPulseTime > 3) { // 3 microseconds debounce
pulseCount++;
lastPulseTime = currentTime;
}
}
// Check for finish line break (called from main loop)
void checkFinishLineBreak() {
static bool lastAlignedState = false;
static unsigned long beamBrokenTime = 0;
static bool beamWasBroken = false;
bool currentLaserState = digitalRead(LASER_SENSOR);
// Detect transition from aligned to not aligned (beam break) during active timing
if (timingActive && lastAlignedState && localAligned && !currentLaserState) {
if (!beamWasBroken) {
beamWasBroken = true;
...
html += "button { background-color: #cccccc; }";
...
status = "Align local laser (freq: " + String(currentFrequency, 0) + "Hz)";
...
// Update laser aligned status with frequency check
...
localAligned = (alignmentCounter >= ALIGNMENT_THRESHOLD);
...
Serial.println("Target frequency: 150kHz +/- 50kHz");
}
void loop() {
unsigned long currentMillis = millis();
...
updateLaserStatus();
// Check for finish line break when timing is active
if (timingActive) {
checkFinishLineBreak();
}
...
Serial.print(localAligned ? "YES" : "NO");
